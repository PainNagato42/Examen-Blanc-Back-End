...
class _model {
    ...
    protected $attributs = [];        //  attributs réels de l'objet du modèle conceptuel
    ...
    function id() {
        // Rôle : récupérer l'id de l'objet courant (ou null si il n'existe pas)
        // Retour : id ou null
        // Paramètres : néant
    ...
    function get($nom) {
        ...
        if (!( isset($this->attributs[$nom]) or $nom === "id")) {
            return null;
        }
        ...
        } else if (isset($this->defaultValues[$nom])) {
            return $this->defaultValues[$nom];
        } else {
            return ["NUM" => 0, "VARCHAR" => "", "TEXT" => "", "LINK" => 0][$this->attributs[$nom]];
        }
    }
    ...
    function getTarget($nom) {
        ...
        if (!isset($this->attributs[$nom]) or $this->attributs[$nom] != "LINK" or!isset($this->target[$nom])) {
    ...
    function update() {
        ...
        $champs = [];
        $param = [];
        foreach ($this->attributs as $nom => $type) {
    ...
    function tabToObj($tab, $classe = null) {
        ...
        //      $classe : classe de l'objet à produire (classe courante par défaut)

        if (is_null($classe)) {
            // On n'a pas fourni de classe : pendre la classe de l'objet courant
    ...
    function sqlToTabObj($sql, $param = [], $classe = null) {
        ...
        //      $classe: classe des objets à générer  (si vide : classe de l'objet courant)
        ...
        while ($ligne = $req->fetch(PDO::FETCH_ASSOC)) {
            // Transformer la ligne en un objet de la classe $classe
            $obj = $this->tabToObj($ligne, $classe);
